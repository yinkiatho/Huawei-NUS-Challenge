...
idx_dict = dict(zip(symbol, list(range(len(symbol))))) #{"symbol" : "idx"}
...
while True:
    ...
    if tm_ms < 13800000:  # before 14:50:00
        ...
        if idx_25min != 0:
            x = np.array([
                np.mean(talib.ATR(np.array(prc_temp[idx_5min:]), timeperiod=5)),
                np.mean(talib.ATR(np.array(prc_temp[idx_10min:]), timeperiod=10)),
                np.mean(talib.ATR(np.array(prc_temp[idx_15min:]), timeperiod=15)),
                np.mean(talib.ATR(np.array(prc_temp[idx_20min:]), timeperiod=20)),
                np.mean(talib.ATR(np.array(prc_temp[idx_25min:]), timeperiod=25)),
